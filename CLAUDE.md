...
**The model does NOT see these domain objects.** `tools/registry/index.ts` no longer spreads them — it exports only `megaTools` from `tools/registry/mega-tools.ts`, which advertises **7 action-dispatched "mega-tools"** (`manage_tasks`, `manage_finances`, `manage_calendar`, `manage_habits`, `manage_memory`, `manage_notes`, `assistant_utils` — 35 actions total) and routes each `action` into the domain `execute()` underneath. Three more blocks (`manage_email`, `manage_photos`, `manage_music`) are **commented out** in `mega-tools.ts`, as are their `HELP_META` entries.
